          ...
          <div className="grid grid-cols-3 md:grid-cols-2 lg:grid-cols-3 gap-2 md:gap-8">
            {filteredDestinations.map((dest, idx) => (
              <motion.div
                ...
                onClick={() => {}}
                className="group block rounded-xl md:rounded-2xl border border-white/10 bg-white/5 backdrop-blur-md overflow-hidden shadow-2xl hover:border-[var(--saffron)]/40 hover:shadow-[0_0_30px_rgba(212,175,55,0.08)] transition-all"
              >
                <Link href={`/destinations/${dest.slug || dest._id}`} className="flex flex-col h-full">
                {/* Image top */}
                ...
                <div className="p-2 md:p-6 flex-1 flex flex-col justify-between">
                  ...
                  <div className="hidden md:flex pt-4 border-t border-white/5 gap-3">
                    <span className="flex-1 text-center bg-white/5 border border-white/10 hover:border-white/30 text-white/90 rounded-xl py-2.5 text-xs font-bold uppercase tracking-widest transition-colors cursor-pointer">
                      View Details
                    </span>
                  </div>
                </div>

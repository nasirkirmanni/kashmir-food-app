...
Generate itinerary using destinations, restaurants, and dishes from the Wazwan Way database.`;
  ...
  return (
    <div className="bg-[#0e0d0b] text-white min-h-screen font-body overflow-x-hidden relative pt-24 pb-12">
      {/* Aesthetic Background */}
      <div className="absolute inset-0 z-0">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,rgba(212,175,99,0.08),transparent_50%)]" />
        <div className="absolute top-1/4 right-0 w-1/2 h-1/2 bg-[radial-gradient(circle_at_right,rgba(200,164,106,0.05),transparent_60%)]" />
        <div className="absolute bottom-0 left-0 w-full h-1/3 bg-gradient-to-t from-black to-transparent" />
      </div>

      <div className="relative z-10 max-w-3xl mx-auto px-6 mb-12 mt-4 md:mt-12">
        <Link href="/plan" className="inline-flex items-center text-white/50 hover:text-white transition-colors mb-6 md:mb-8 text-sm font-bold uppercase tracking-widest bg-white/5 px-4 py-2 rounded-full border border-white/10 hover:border-white/30">
          &larr; Back
        </Link>
        <div className="text-center">
          <h1 className="text-4xl md:text-5xl font-display font-medium text-white mb-4">Plan Your Custom Trip</h1>
          <p className="text-white/60 text-lg max-w-xl mx-auto">Tell us a bit about your travel style, and we'll craft a bespoke Kashmir experience just for you.</p>
        </div>
      </div>

      {/* FLOATING AI PLANNER */}
      <div className="relative z-30 w-full max-w-[900px] mx-auto px-4 md:px-12 mb-32" id="planner-section">

...
export default function GlobalBackground() {
  ...
  const homeBlur = useTransform(scrollY, [0, 300, 600, 900], ["blur(0px)", "blur(2px)", "blur(4px)", "blur(6px)"]);
  const homeGradientOpacity = useTransform(scrollY, [0, 600], [1, 0.85]);

  const opacity = isHome ? homeOpacity : 0.15;
  const filter = isHome ? homeBlur : "blur(6px)";
  const gradientOpacity = isHome ? homeGradientOpacity : 0.8;

  return (
    <div className="fixed inset-0 z-[-1] pointer-events-none bg-[#0B0B0B]">
      <motion.img 
        src="/wazwan-hero-mobile.png" 
        alt="" 
        style={{ opacity, filter }}
        className="block md:hidden h-full w-full object-cover object-bottom scale-[1.05]" 
      />
      <motion.div 

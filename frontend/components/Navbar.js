...
export default function Navbar() {
  ...
  return (
    <>
      {desktopNav}
      {/* Mobile Nav Bar */}
      <nav className="md:hidden fixed top-0 left-0 right-0 z-50 w-full border-b border-white/10 bg-[#0B0B0B]/80 backdrop-blur-xl" style={{ paddingTop: "env(safe-area-inset-top)" }}>
        <div className="flex h-24 items-center justify-between w-full px-6 lg:px-12 2xl:px-16">
          ...
          <div className="flex items-center gap-2">
            {user ? (
              ...
              <>
                <Link href="/login" className="text-xs font-bold uppercase tracking-widest text-white transition-colors hover:text-[#C8A46A] whitespace-nowrap">
                  Login
                </Link>
                <Link href="/signup" className="rounded-full bg-[#C8A46A] px-5 py-2.5 text-xs font-bold uppercase tracking-widest text-black shadow-[0_0_20px_rgba(212,175,55,0.2)] transition-transform hover:scale-105 whitespace-nowrap">
                  Sign up
                </Link>
              </>
            )}

...
**Follow-up releases:** Blog is in the top bar, the menu drawer and the phone menu, and each food hub page (`/kashmiri-food/wazwan`, `/bakery`, `/beverages` and `/street-food`) has its own heading, intro and buttons, each linking to its matching guide.

...
export default function PlanTripPage() {
  ...
  const generateItinerary = (params, db) => {
    ...
    for (let d = 1; d <= numDays; d++) {
      ...
      // Budget indicator
      const budgetMap = { Budget: "Γé╣", "Mid-Range": "Γé╣Γé╣", Premium: "Γé╣Γé╣Γé╣", Luxury: "Γé╣Γé╣Γé╣Γé╣" };
      const estBudget = budgetMap[budget] || "Γé╣Γé╣";
    ...
    const totalEstCost = `Γé╣${(costPerDay * numDays * (party === "Solo Traveler" ? 1 : party === "Couple" ? 1.8 : 3.5)).toLocaleString()} - Γé╣${(costPerDay * numDays * 1.5 * (party === "Solo Traveler" ? 1 : party === "Couple" ? 1.8 : 3.5)).toLocaleString()}`;
                    ...
                    return (
                      ...
                      >
                        <div className="text-sm font-bold flex justify-between items-center w-full">
                          ...
                          {isSelected && <span className="text-xs">Γ£ô</span>}
                        </div>
                      ...
                      <ul className="space-y-1.5 text-white/80">
                        {result.summary.photoSpots.map((spot, idx) => (
                          <li key={idx} className="flex items-start gap-1.5">
                            <span className="text-[var(--saffron)]">ΓÇó</span>
                            <span>{spot}</span>
                          </li>
                        ))}
                      </ul>
                    ...
                    <ul className="space-y-3">
                      {result.spots.map((spot, idx) => (
                        <li key={idx} className="flex gap-2.5 items-start text-xs text-white/85">
                          <span className="text-[var(--saffron)] font-bold text-sm leading-none">ΓÇó</span>
                          <div>
